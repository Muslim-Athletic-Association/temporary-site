    ...
    for (var i = 0; i < terms.length; i++) {
        if ($(terms[i]).is(":visible") && $(terms[i]).is(":required")) {
            if (!$(terms[i]).is(":checked")) {
                ...
                $('#alert-message').html( 'Please select all required consent checks. <br> Please contact us at info@maaweb.org if you think there is an issue.')
                $('#alert').slideDown();            
                return false;
            }
        }
...
function register(member) {
    ...
    if ($("#first_name").val() == "" || $("#last_name").val() == ""){
        $('#alert-message').html("Please fill in your first and last name." + '<br> Please contact us at info@maaweb.org if you think there is an issue.')
        $('#alert').slideDown();
        return false;
    }
    ...
    $.ajax({
        ...
        data: {
            ...
            subscription: 2,
            consent: [{ "given": "true", "purpose": "Ramadan Agreement" }, { "given": email_consent, "purpose": "Ramadan Sponsor Contact" }],
            guardian: {
